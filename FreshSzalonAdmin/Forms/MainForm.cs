    ...
    public partial class MainForm : MaterialForm
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (adatbazis.TestConnection() == true)
            {
                ...
                KategoriakFulRendezese();
                TermekekFulRendezese();
            }
        }
        ...
        private void VezerlopultRendezese()
        {
            ...
            {
                kartya.Anchor = AnchorStyles.Top | AnchorStyles.Left;
        ...
        private void DolgozokFulRendezese()
        {
            ...
            btnHozzarendeles.Anchor = AnchorStyles.Bottom | AnchorStyles.Left;
            btnHozzarendeles.Location = new System.Drawing.Point(btnTorles.Right + padding, szuloFul.ClientSize.Height - btnHozzarendeles.Height - padding);
        ...
        private void naptarFulRendezese()
        {
            ...
            dgvFoglalasok.BackgroundColor = System.Drawing.Color.White;
        }
        ...
        private void KategoriakFulRendezese()
        {
            Control szuloFul = dgvKategoriak.Parent;
            if (szuloFul == null) return;

            int padding = 20;

            btnFrissitKategoria.Location = new System.Drawing.Point(padding, 20);
            btnFrissitKategoria.Anchor = AnchorStyles.Top | AnchorStyles.Left;

            btnUjKategoria.Anchor = AnchorStyles.Bottom | AnchorStyles.Left;
            btnUjKategoria.Location = new System.Drawing.Point(padding, szuloFul.ClientSize.Height - btnUjKategoria.Height - padding);

            btnSzerkesztKategoria.Anchor = AnchorStyles.Bottom | AnchorStyles.Left;
            btnSzerkesztKategoria.Location = new System.Drawing.Point(btnUjKategoria.Right + padding, btnUjKategoria.Top);

            btnTorolKategoria.Anchor = AnchorStyles.Bottom | AnchorStyles.Left;
            btnTorolKategoria.Location = new System.Drawing.Point(btnSzerkesztKategoria.Right + padding, btnUjKategoria.Top);

            dgvKategoriak.Location = new System.Drawing.Point(padding, btnFrissitKategoria.Bottom + 20);
            dgvKategoriak.Width = szuloFul.ClientSize.Width - (padding * 2);
            dgvKategoriak.Height = btnUjKategoria.Top - dgvKategoriak.Top - 20;

            dgvKategoriak.Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Right;

            dgvKategoriak.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            dgvKategoriak.BackgroundColor = System.Drawing.Color.White;
            dgvKategoriak.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            dgvKategoriak.ReadOnly = true;
            dgvKategoriak.AllowUserToAddRows = false;
        }

        private void TermekekFulRendezese()
        {
            Control szuloFul = dgvTermekek.Parent;
            if (szuloFul == null) return;
            int padding = 20;

            btnFrissitTermekek.Location = new System.Drawing.Point(padding, 20);
            btnUjTermek.Location = new System.Drawing.Point(padding, szuloFul.ClientSize.Height - btnUjTermek.Height - padding);
            btnSzerkesztTermek.Location = new System.Drawing.Point(btnUjTermek.Right + padding, btnUjTermek.Top);
            btnTorolTermek.Location = new System.Drawing.Point(btnSzerkesztTermek.Right + padding, btnUjTermek.Top);

            dgvTermekek.Location = new System.Drawing.Point(padding, btnFrissitTermekek.Bottom + 20);
            dgvTermekek.Width = szuloFul.ClientSize.Width - (padding * 2);
            dgvTermekek.Height = btnUjTermek.Top - dgvTermekek.Top - 20;

            dgvTermekek.Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Right;
            btnUjTermek.Anchor = btnSzerkesztTermek.Anchor = btnTorolTermek.Anchor = AnchorStyles.Bottom | AnchorStyles.Left;

            dgvTermekek.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            dgvTermekek.BackgroundColor = System.Drawing.Color.White;
            dgvTermekek.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            dgvTermekek.ReadOnly = true;
            dgvTermekek.AllowUserToAddRows = false;
        }
        ...
        {
            dgvKategoriak.DataSource = adatbazis.GetKategoriak();
        }

        private void TermekekBetoltese()
        {
            dgvTermekek.DataSource = adatbazis.GetTermekek();
            if (dgvTermekek.Columns.Contains("lehetosegek_id"))
                dgvTermekek.Columns["lehetosegek_id"].Visible = false;
        }
        ...
        private void LemondasEmailKuldese(string cimzettEmail, string vendegNev, string datum, string szolgaltatasNev)
        {
            try
            {
                ...
                mailMessage.To.Add(cimzettEmail);
        ...
        private void btnHozzarendeles_Click(object sender, EventArgs e)
        {
            SzolgaltatokForm f = new SzolgaltatokForm();
            f.ShowDialog();
        }

        private void btnFrissitKategoria_Click(object sender, EventArgs e)
        {
           KategoriakBetoltese();
        }

        private void btnUjKategoria_Click(object sender, EventArgs e)
        {
            UjKategoriaForm ujForm = new UjKategoriaForm();
        ...
        private void btnSzerkesztKategoria_Click(object sender, EventArgs e)
        {
            if (dgvKategoriak.SelectedRows.Count > 0)
            {
                int id = Convert.ToInt32(dgvKategoriak.SelectedRows[0].Cells[0].Value);
                string nev = dgvKategoriak.SelectedRows[0].Cells[1].Value.ToString();
                string leiras = dgvKategoriak.SelectedRows[0].Cells[2].Value.ToString();

                KategoriaSzerkesztoForm szerkForm = new KategoriaSzerkesztoForm(id, nev, leiras);
                if (szerkForm.ShowDialog() == DialogResult.OK)
                {
                    KategoriakBetoltese();
                }
            }
            else { MessageBox.Show("Válassz ki egy kategóriát!"); }
        }

        private void btnTorolKategoria_Click(object sender, EventArgs e)
        {
            if (dgvKategoriak.SelectedRows.Count > 0)
            {
                int id = Convert.ToInt32(dgvKategoriak.SelectedRows[0].Cells[0].Value);
                ...
                if (MessageBox.Show($"Biztosan törlöd a(z) {nev} kategóriát?", "Törlés", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    adatbazis.KategoriaTorles(id);
                    KategoriakBetoltese();
                }
            }
            else { MessageBox.Show("Válassz ki egy kategóriát!"); }
        ...
        {
            UjTermekekForm ujTermekForm = new UjTermekekForm();

            if (ujTermekForm.ShowDialog() == DialogResult.OK)
            {
                TermekekBetoltese();
            }
        }

        private void btnFrissitTermekek_Click(object sender, EventArgs e)
        {
            TermekekBetoltese();
        }

        private void btnTorolTermek_Click(object sender, EventArgs e)
        {
            if (dgvTermekek.SelectedRows.Count > 0)
            {
                int id = Convert.ToInt32(dgvTermekek.SelectedRows[0].Cells[0].Value);
                string nev = dgvTermekek.SelectedRows[0].Cells[1].Value.ToString();

                var valasz = MessageBox.Show($"Biztosan törölni akarod a(z) {nev} terméket?", "Törlés megerősítése",
                                             MessageBoxButtons.YesNo, MessageBoxIcon.Warning);

                if (valasz == DialogResult.Yes)
                {
                    adatbazis.TermekTorles(id);
                    TermekekBetoltese();
                    MessageBox.Show("Termék sikeresen törölve!", "Siker", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
                MessageBox.Show("Kérlek, válassz ki egy terméket a táblázatból!", "Figyelmeztetés", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }

        private void btnSzerkesztTermek_Click(object sender, EventArgs e)
        {
            if (dgvTermekek.SelectedRows.Count > 0)
            {
                int id = Convert.ToInt32(dgvTermekek.SelectedRows[0].Cells[0].Value);
                string nev = dgvTermekek.SelectedRows[0].Cells[1].Value.ToString();
                string leiras = dgvTermekek.SelectedRows[0].Cells[2].Value.ToString();
                int ar = Convert.ToInt32(dgvTermekek.SelectedRows[0].Cells[3].Value);
                int kategoriaId = Convert.ToInt32(dgvTermekek.SelectedRows[0].Cells["lehetosegek_id"].Value);

                TermekSzerkesztoForm szerkForm = new TermekSzerkesztoForm(id, nev, leiras, ar, kategoriaId);
                if (szerkForm.ShowDialog() == DialogResult.OK)
                {
                    TermekekBetoltese();
                }
            ...
            {
                MessageBox.Show("Kérlek, válassz ki egy terméket a szerkesztéshez!", "Figyelmeztetés", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    }
}
