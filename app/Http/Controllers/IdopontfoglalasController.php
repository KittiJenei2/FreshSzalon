...
class IdopontfoglalasController extends Controller
{
    ...
    public function SzabadIdopontok(Request $request)
    {
        ...
        // 4️⃣ Már lefoglalt időpontok (KIVÉVE az elutasítottakat, statuszok_id != 3)
        $foglalasok = Idopontfoglalas::where('dolgozo_id', $dolgozoId)
            ->whereDate('datum', $datum)
            ->where('statuszok_id', '!=', 3) // Ne blokkolja a naptárat, amit már elutasítottak
    ...
    public function store(Request $request)
    {
        ...
        // Időpont vége kiszámolva
        $idoKezdesCarbon = Carbon::parse($request->ido_kezdes);
        $idoVegCarbon = $idoKezdesCarbon->copy()->addMinutes($szolgaltatas->idotartam);

        // -- ÚJ: SZERVEROLDALI ÁTFEDÉS VIZSGÁLAT --
        $utkozik = Idopontfoglalas::where('dolgozo_id', $request->dolgozo_id)
            ->whereDate('datum', $request->datum)
            ->where('statuszok_id', '!=', 3) // Az elutasított foglalásokkal ütközhet
            ->where(function ($query) use ($idoKezdesCarbon, $idoVegCarbon) {
                $query->where('ido_kezdes', '<', $idoVegCarbon->format('H:i:s'))
                      ->where('ido_vege', '>', $idoKezdesCarbon->format('H:i:s'));
            })
            ->exists();

        if ($utkozik) {
            return response()->json([
                'uzenet' => 'Sajnos ez az időpont időközben betelt! Kérlek, válassz másikat.'
            ], 422); // A 422-es hiba miatt a frontend hibát fog jelezni a felhasználónak
        }
        // -----------------------------------------
