...
<tbody>
                                    @foreach($aktualis_foglalasok as $f)
                                        {{-- A sor háttere szürke lesz, ha elutasítva (3) van --}}
<tr class="{{ $f->statuszok_id == 3 ? 'bg-light' : '' }}" style="transition: background-color 0.3s ease;">
                                            {{-- Az első 5 oszlop tartalmát halványítjuk el, így a gomb oszlopa eredeti marad --}}
<td class="fw-bold {{ $f->statuszok_id == 3 ? 'opacity-50' : '' }}">{{ $f->datum }}</td>
<td class="{{ $f->statuszok_id == 3 ? 'opacity-50' : '' }}">{{ substr($f->ido_kezdes, 0, 5) }}</td>
<td class="{{ $f->statuszok_id == 3 ? 'opacity-50' : '' }}">{{ $f->szolgaltatas->nev }}</td>
<td class="{{ $f->statuszok_id == 3 ? 'opacity-50' : '' }}">{{ $f->dolgozo->nev }}</td>
<td class="{{ $f->statuszok_id == 3 ? 'opacity-50' : '' }}">
                                                {{-- Státusz badge színeinek meghatározása --}}
                                                @php
                                            ...
                                            {{-- Az utolsó oszlop (Műveletek) NINCS elhalványítva --}}
<td class="text-end pe-4">
<div class="d-flex justify-content-end gap-2">
 
                                                    {{-- Lemondás és Módosítás gombok aktív foglalásokhoz (Függőben=1, Elfogadva=2) --}}
                                                    @if(in_array($f->statuszok_id, [1, 2]))
<button class="btn btn-sm btn-outline-warning rounded-pill fw-bold modifyBtn"
...
<button class="btn btn-sm btn-outline-danger rounded-pill fw-bold lemondBtn"
                                                                data-id="{{ $f->id }}">
                                                            ❌ Lemondás
</button>
                                                    @endif
                                                    {{-- Piros figyelmeztetés és gomb, ha el lett utasítva --}}
                                                    @if($f->statuszok_id == 3)
                                                        @php
...
<span class="text-muted">({{ $szabadsagMiatta->datum_kezdes }} / {{ $szabadsagMiatta->datum_vege }})</span><br>
                                                            @else
<i class="fw-bold">Sajnáljuk, ez az időpont betelt!</i><br>
                                                                Kérünk, válassz másikat!
                                                            @endif
<div class="mt-1">
<a href="{{ route('idopontfoglalas.index') }}" class="btn btn-sm btn-outline-danger py-0 px-2 fw-bold" style="font-size: 0.7rem;">
                                                                    Új foglalás →
</a>
</div>
</div>
